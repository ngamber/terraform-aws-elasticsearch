...
resource "aws_cloudwatch_metric_alarm" "cluster_state" {
  count                     = var.enable_cw_alarms ? 1 : 0
  alarm_name                = "${var.namespace}-${var.stage}-elasticsearch-cluster_state"
  ...
  threshold                 = "1"
  alarm_description         = "Cluster is no longer green"
  alarm_actions             = [var.pagerduty_topic_arn]
  ok_actions                = [var.pagerduty_topic_arn]
  insufficient_data_actions = []

  dimensions = {
    DomainName = "${var.namespace}-${var.stage}-es",
...
resource "aws_cloudwatch_metric_alarm" "writes_blocked" {
  count                     = var.enable_cw_alarms ? 1 : 0
  alarm_name                = "${var.namespace}-${var.stage}-elasticsearch-writes_blocked"
  ...
  datapoints_to_alarm       = "4"
  threshold                 = "0"
  alarm_description         = "Indexing is blocked"
  alarm_actions             = [var.pagerduty_topic_arn]
  ok_actions                = [var.pagerduty_topic_arn]
  insufficient_data_actions = []
...
resource "aws_cloudwatch_metric_alarm" "snapshot_failure" {
  count                     = var.enable_cw_alarms ? 1 : 0
  alarm_name                = "${var.namespace}-${var.stage}-elasticsearch-snapshot_failure"
  ...
  datapoints_to_alarm       = "4"
  threshold                 = "0"
  alarm_description         = "Snapshotting has failed"
  alarm_actions             = [var.pagerduty_topic_arn]
  ok_actions                = [var.pagerduty_topic_arn]
  insufficient_data_actions = []
